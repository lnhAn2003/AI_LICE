...
const EditPage: React.FC<EditPageProps> = ({ courseData, id, allCategories }) => {
  ...
  const confirmUpdate = async () => {
    ...
    try {
      await axiosInstance.patch(`/courses/${id}`, pendingFormData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      alert('Course updated successfully!');
      router.push(`/studio/${courseData.createdBy._id}`);
    } catch (error: any) {
    ...
    <div className="max-w-4xl mx-auto mt-10 px-4 pb-10">
      ...
      <EditCourseForm
        initialData={{
          ...
          categories: courseData.categories.map((cat) => cat._id),
