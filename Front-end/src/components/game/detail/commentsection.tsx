...
import { Comment, Reply, CommentsSectionProps } from '../../../types/game';
import axiosInstance from '../../../utils/axiosInstance';
import { useAuth } from '../../../hooks/useAuth';
import { FiThumbsUp, FiMessageCircle, FiMoreHorizontal } from 'react-icons/fi';

const CommentsSection: React.FC<CommentsSectionProps> = ({ comments, targetType, targetId }) => {
  ...
  const renderComments = () => {
    return currentComments.map(comment => (
      <div key={comment._id} className="mb-8 px-4 md:px-8">
        {/* Comment Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <img
              src={comment.authorId?.profile?.avatarUrl || '/default-avatar.png'}
              alt={comment.authorId?.username || 'Anonymous'}
        ...
        {openReplyInput === comment._id && (
          <div className="mt-4">
            <label htmlFor={`reply-${comment._id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
              Write a reply:
            </label>
            <textarea
  ...
  return (
    <div className="w-full">
      ...
      {/* Add Comment */}
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-8 transition-colors duration-300 w-full">
